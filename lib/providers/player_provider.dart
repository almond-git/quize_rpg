...
class PlayerProvider with ChangeNotifier {
  ...
  Future<void> createNewPlayer(String name) async {
    ...
    try {
      ...
      try {
        int id = await createPlayer(name);
        _player!.id = id;
  ...
  Future<bool> gainExperience(int amount) async {
    ...
    // 플레이어 상태 저장
    try {
      await _preferenceService.updatePlayer(_player!);
    } catch (e) {
  ...
  Future<void> addWrongQuestion(int questionId) async {
    ...
    try {
      await _preferenceService.addWrongQuestion(_player!.id, questionId);
    } catch (e) {
  ...
  Future<void> removeWrongQuestion(int questionId) async {
    ...
    try {
      await _preferenceService.removeWrongQuestion(_player!.id, questionId);
    } catch (e) {
  ...
  Future<bool> deletePlayer(int id) async {
    ...
    try {
      ...
      // 데이터베이스에서 플레이어 삭제
      final result = await _preferenceService.deletePlayer(id);
  ...
  Future<void> loadPlayer(int id) async {
    final player = await _preferenceService.getPlayerById(id);
    if (player != null) {
      _player = player;
      notifyListeners();
    }
  ...
  Future<List<Player>> getPlayers() async {
    return await _preferenceService.getPlayers();
  }

  Future<int> createPlayer(String name) async {
    final newPlayer = Player(
      id: 0, // ID는 서비스에서 자동 할당
      name: name,
      level: 1,
      experience: 0,
      requiredExperience: 100,
    ...
    final playerId = await _preferenceService.addPlayer(newPlayer);
